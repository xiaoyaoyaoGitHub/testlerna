import { _support, validateOption, logger, isBrowserEnv, isWxMiniEnv, variableTypeDetection, Queue, isEmpty } from '@gymito/monitor-utils'
import { createErrorId } from './errorId'
import { SDK_NAME, SDK_VERSION } from '@gymito/monitor-shared'
import { breadcrumb } from './breadcrumb'
import { AuthInfo, TransportDataType, EMethods, InitOptions, isReportDataType, DeviceInfo, FinalReportType } from '@gymito/monitor-types'
/**
...
export class TransportData {
   ...
   * @returns
   */
  async send(data: FinalReportType | any) {
    let dsn = ''
    if (isReportDataType(data)) {
      dsn = this.errorDsn
      if (isEmpty(dsn)) {
